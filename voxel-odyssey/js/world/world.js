...
export class World {
  ...
  getGroundSpawn(wx, wz) {
    ...
    // With realistic ocean coverage the requested column is underwater on a
    // fair share of seeds, and spawning a new player treading water is a
    // miserable first minute. Spiral outward over the DETERMINISTIC worldgen
    // height (cheap — no chunks are generated by this) and spawn on the
    // nearest dry column instead. The ring step keeps the scan coarse; the
    // exact centre column is still preferred when it is already land.
    const wg = this.game && this.game.worldgen;
    if (wg && typeof wg.heightAt === 'function'
        && wg.heightAt(wx, wz) <= WATER_LEVEL) {
      outer:
      for (let r = 8; r <= 256; r += 8) {
        for (let a = 0; a < 16; a++) {
          const ang = (a / 16) * Math.PI * 2;
          const sx = wx + Math.round(Math.cos(ang) * r);
          const sz = wz + Math.round(Math.sin(ang) * r);
          if (wg.heightAt(sx, sz) > WATER_LEVEL + 1) { wx = sx; wz = sz; break outer; }
        }
      }
      // No land within 256 blocks: an ocean seed. Fall through and swim.
    }
